...
void setup() {
  ...
  for (int amp_knob = 0; amp_knob < num_knobs; amp_knob++) {
    ...
    attachInterrupt(digitalPinToInterrupt(knobs[i]._pin), button_pressed, CHANGE);  //  function for creating external interrupts at pin2 on Change
  }
} 
...
class Knob(knob amp_knob) {
  ...
  void switch() {
    ...
    digitalWrite(_switch, _switch_state);
